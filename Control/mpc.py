...
class KinematicModel_3:
  ...
  def update_nonlinear(self, v_c, phi_c):

    self.x = self.x+self.v*math.cos(self.psi)*self.dt
    self.y = self.y+self.v*math.sin(self.psi)*self.dt
    self.psi = self.psi+self.v/self.L*math.tan(phi_c)*self.dt
    self.v = 1
...
class MyReferencePath:
    def __init__(self):
        ...
        self.refer_path = np.zeros((100, 4))
        self.refer_path[:, 0] = np.linspace(0, 50, 100)  # x
        # self.refer_path[:, 1] = 2*np.sin(self.refer_path[:, 0]/5.0) + \
        #     0.8*np.cos(self.refer_path[:, 0]/2.0)  # y
        self.refer_path[:, 1] = 2
...
def linear_mpc_control(xref, x0, delta_ref, ugv):
    ...
    state_e = x[:,0] - xref[:,0]
    ctrl_e = u[:,0] - delta_ref[:,0]
    cost += cvxpy.quad_form(x[:, T] - xref[:, T], Qf)

    constraints += [(x[:, 0]) == x0]
    constraints += [(x[:,T-1])==0.001]
...
def nonlinear_ctrl(ugv, ref_point, k1 = 1, k2= 0.8, dstar = 1.2):
    distance = np.sqrt(np.power(ref_point[0]-ugv.x,2)+np.power(ref_point[1]-ugv.y,2))
    atan_angle = np.arctan((ref_point[1]-ugv.y)/(ref_point[0]-ugv.x))
    
    down = np.sqrt(np.power(1/L,2)+np.power(np.cos(atan_angle)/distance,2))
    tmp_gamma = (ugv.v * atan_angle * np.cos(atan_angle) )/ (distance * down)
    tmp_yta = np.cos(atan_angle) /(distance * down)
    dis_error = distance - dstar
    phi = np.arcsin(-k2*tmp_gamma) + np.arccos(tmp_yta)
    v = -k1 * np.sin(atan_angle) * np.sin(phi)*(distance - dstar)
    return v,phi,dis_error
...
def main():
    ...
    fig = plt.figure(1)
    index = 4
    goal = reference_path.refer_path[index, 0:2] #第一个目标点
    for i in range(100):
        v_c, phi_c,de= nonlinear_ctrl(ugv, goal)
        ugv.update_nonlinear(v_c,phi_c)
        x_.append(ugv.x)
        y_.append(ugv.y)
    ...
    plt.plot(reference_path.refer_path[:, 0], reference_path.refer_path[:,
          1], "-.b",  linewidth=1.0, label="reference")  
    plt.plot(x_,y_,"-r",label="real") 
    plt.xlabel("x-axis")
    plt.ylabel("y-axis")
    plt.title("Tracking Simulation") 
...
def main_mpc():
    ...
    reference_path = MyReferencePath()
    goal = reference_path.refer_path[-1, 0:2] #第一个目标点
    ...
    for i in range(500):
        ...
        x_.append(ugv.x)
        y_.append(ugv.y)
        # x_e.append(ugv.x - xref[0,i])
        # y_e.append(ugv.y - xref[1,i])
        # psi_e.append(ugv.psi - xref[2,i])
        # 显示动图
        # plt.cla()
        # plt.plot(reference_path.refer_path[:, 0], reference_path.refer_path[:,
    ...
    lin_t = np.linspace(0, 50, len(track_e))
    # lin_s = np.linspace(0, 50, len(state_e))
    # lin_u = np.linspace(0, 50, len(ctrl_e))
    plt.figure(2)
    plt.plot(lin_t, track_e, "-g")
    # plt.plot(lin_s, state_e, "-r")
    # plt.plot(lin_u, ctrl_e, "-b")
    # plt.plot(lin, y_e, '-r')
    # plt.plot(lin, psi_e, '-b')
    plt.xlabel("points")
    plt.ylabel("errors")
    plt.title("Tracking Errors") 
    plt.legend(['distance - errors'])
    plt.grid()
    plt.show()  
